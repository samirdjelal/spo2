...
use self::routes::{delete_url, get_all_urls, read_url, update_url};
...
const DISCORD_HOOK_URL: &str = "DISCORD_HOOK_URL";
...
fn main() -> Result<(), io::Error> {
    ...
    runtime.spawn_ok(async move {
        let discord_hook_url = match env::var(DISCORD_HOOK_URL) {
            Ok(url) => url,
            Err(e) => {
                eprintln!("{}: {}", DISCORD_HOOK_URL, e);
                String::new()
            }
        };
        ...
        while let Some(mut reports) = receiver.next().await {
            ...
            if discord_hook_url != "" {
                let body = serde_json::json!({"username": "samir", "content": body });
                let request = Request::post(&discord_hook_url)
    ...
    let _ = thread::spawn(|| ws.listen(ws_listen_addr).expect("websocket listen error"));
    ...
    for result in database.iter() {
        ...
        runtime
            .spawn_ok(async { health_checker(url, notifier_sender, event_sender, database).await });
    }
